...
export class DynamicRouting {
  private static readonly DEFAULT_HANDLER_FILE = "api";
  private static readonly DEFAULT_EXTENSIONS = ["ts", "js"];
  private static readonly SUPPORTED_METHODS: Method[] = [
    "GET",
    "HEAD",
    "POST",
    "PUT",
  ...
  private static normalize_extensions(extensions?: string[]): string[] {
    const source =
    ...
    return result.length ? result : this.DEFAULT_EXTENSIONS;
  }

  private static to_route_path(root_dir: string, file_path: string): string {
  ...
  private static is_dynamic_handler_file(
    file_name: string,
    handler_file: string,
    extensions: string[],
  ): boolean {
  ...
  private static async collect_dynamic_route_files(
    root_dir: string,
    handler_file: string,
    extensions: string[],
  ): Promise<string[]> {
    ...
    while (i < dirs.length) {
      ...
      while (e < entries.length) {
        ...
        if (this.is_dynamic_handler_file(entry.name, handler_file, extensions))
  ...
  private static async import_dynamic_module(
    file_path: string,
  ...
  private static get_conflict_policy(
  ...
  private static has_supported_static_methods(
    target: unknown,
  ): target is DynamicRouteClass {
    if (typeof target !== "function") return false;

    let i = 0;
    while (i < this.SUPPORTED_METHODS.length) {
      const method = this.SUPPORTED_METHODS[i++] as Method;
  ...
  private static resolve_route_class(
    route_module: DynamicRouteModule,
  ): DynamicRouteClass | null {
    if (this.has_supported_static_methods(route_module.default))
    ...
    while (i < exports.length) {
      ...
      if (this.SUPPORTED_METHODS.includes(export_name as Method)) continue;

      const export_value = route_module[export_name];
      if (this.has_supported_static_methods(export_value)) return export_value;
    }

    return null;
  }

  private static resolve_method_handler(
    route_module: DynamicRouteModule,
  ...
  static async register_dynamic_routes(
    router: Router,
    options: DynamicRoutingOptions,
  ): Promise<number> {
    if (typeof options !== "object" || options === null)
      throw new TypeError(
        "[Oxarion] dynamicRouting: options must be an object",
      );
    ...
    const handler_file =
      typeof options.handlerFile === "string" && options.handlerFile
        ? options.handlerFile
        : this.DEFAULT_HANDLER_FILE;
    const extensions = this.normalize_extensions(options.extensions);
    const conflict_policy = this.get_conflict_policy(options.onConflict);

    const route_files = await this.collect_dynamic_route_files(
      dynamic_dir,
      handler_file,
      extensions,
    );
    ...
    while (f < route_files.length) {
      ...
      const route_path = this.to_route_path(dynamic_dir, file_path);
      const route_module = await this.import_dynamic_module(file_path);
      const route_class = this.resolve_route_class(route_module);

      let found_method = false;
      let m = 0;

      while (m < this.SUPPORTED_METHODS.length) {
        const method = this.SUPPORTED_METHODS[m++];
        const resolved_method = this.resolve_method_handler(
          route_module,
          route_class,
          method,
        );
        ...
        const { handler: method_handler, owner: method_owner } =
          resolved_method;
